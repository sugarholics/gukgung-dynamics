...
## 9. 현재 검증 상태 (2026-04-03)

### 9.1 동작 확인

- [x] 활채 q 감소 (만작→brace 방향): 0.600→0.005 in 20.7ms ✓
- [x] 시위 분리 자동 감지: t_sep = 20.69ms ✓
- [x] 재접촉 에러 없음 ✓
- [x] 모달 주파수 57.8 Hz ✓
- [x] v_arrow = -37.7 m/s ✓ (Klopsteg η=0.329)
- [x] CoM vy ≈ 0 ✓
- [x] 모달 A1 = 0.1 mm ✓ (강체 병진 모델, 횡력 미적용 상태)
- [x] 에너지 보존: KE_arrow(17.8J) + KE_limb(31.9J) = 49.7J ≈ E_stored(54.1J) × 0.92 ✓

### 9.2 해결 완료 (2026-04-03)

| 문제 | 수정 전 | 수정 후 | 수정 내용 |
|------|---------|---------|----------|
| v_arrow | -19.1 m/s | -37.7 m/s | Klopsteg 결합 질량 (m_coupled = m_eff + m_arrow) |
| CoM vy | +9.17 m/s | 0.00 m/s | 강체 병진 + L_upper/L_lower 정합성 보장 |
| 모달 A1 | 81.7 mm | 0.1 mm | SHAKE 수렴 → 강체 병진 + 굽힘 섭동 방식 |

### 9.3 남은 과제

1. **Archer's paradox**: 현재 on-string 동안 순수 강체 병진 → 횡변위 미발생. 시위 각도 비대칭에 의한 횡력을 추가하면 A1 ≈ 3~8 mm 예상.
2. **분리 시점 검증**: t_sep = 20.69ms가 물리적으로 적절한지 확인 필요 (q가 거의 0에 도달해 분리).
3. **동적 T 분리 판정**: 현재 Klopsteg 비례 T 사용. nock 클립 분리는 횡력 기반이므로 향후 횡력 모델 추가 시 재검토.
