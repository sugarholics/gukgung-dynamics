...
function preSampleBowAnchors(params, n = 21) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // 활채 비틂에 의한 앵커 z-보정 (정적 준정적 근사)
    // M_twist = T × z_arrow × sin(시위-고자 각도) ≈ T × z_arrow × (nockY_range / distAnchor)
    const z_arrow_val = -((params.limbWidth || 0.028) / 2 + 0.003); // 화살 z위치
    const GJ = params.torsionalGJ || 33.4;
    const limbArcLen = (params.bowLength / 2 - params.gripLength / 2 - params.siyahLength);
    const T_cur = g.T_current || 0;
    // 시위 방향의 z-성분이 만드는 비틂 모멘트
    const M_twist = Math.abs(T_cur * z_arrow_val * (L_upper > 0.01 ? (nockPt.y - anchorTop.y) / L_upper : 0));
    const phi_twist = limbArcLen > 0 ? M_twist * limbArcLen / GJ : 0;
    const dz_anchor = phi_twist * (params.siyahLength || 0.08); // 고자 끝의 z-편향
    ...
    samples.push({
      ...
      L_upper, L_lower,
      dz_anchor_twist: dz_anchor, // 비틂에 의한 앵커 z-편향 (mm)
      phi_twist_deg: phi_twist * 180 / Math.PI, // 비틂각 (°)
...
function interpolateBowByQ(samples, q) {
  ...
  return {
    ...
    restPoint: s0.restPoint,
    dz_anchor_twist: lerp(s0.dz_anchor_twist || 0, s1.dz_anchor_twist || 0),
    q,
  };
}
...
function simulateRelease(params) {
  ...
  // 활채 유효질량 계수: tapered beam 1차 모드형상 적분 0.123
  // h(s)=h₀(1-0.55s/L), w(s)=w₀(1-0.3s/L) → 정적 처짐 모드 적분
  // 균일 보 0.236 → 테이퍼 보 0.123 (질량이 근부 집중, 선단 경량)
  // 시위 체인은 앵커+nock에 핀 → Klopsteg 유효질량에 포함 유지
  const m_eff_limb = 2 * (0.123 * m_limb_each + m_siyah_each) + m_string / 3;
  ...
  for (let step = 0; step <= totalSteps; step++) {
    ...
    // 3D 앵커 (시위 앵커 = 활 중심면 z=0 + 비틂 보정)
    const dz_twist = limbState.dz_anchor_twist || 0;
    const anchorTop3d = { x: anchorTop.x, y: anchorTop.y, z: dz_twist };
    const anchorBot3d = { x: anchorBot.x, y: anchorBot.y, z: -dz_twist };
