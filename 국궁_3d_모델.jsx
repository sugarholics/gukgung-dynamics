...
function simulateRelease(params) {
  ...
  // 시위 상현/하현 고정 길이 (만작 시점의 값 사용)
  const L_upper = fullDraw.L_upper;
  const L_lower = fullDraw.L_lower;
  ...
  for (let step = 0; step <= totalSteps; step++) {
    ...
    if (arrowState.onString) {
      ...
      // T = 정적 평형 장력 (q에서의 시위 장력, 사전 샘플에서 보간)
      // 동적 보정은 무시 (정적 T가 1차 근사로 충분)
      const T_string = limbState.T_current;

      // ── D. 화살에 시위력 적용 (Fx + Fy 모두) ──
      const Fx = T_string * e_sum_x;
      const Fy = T_string * e_sum_y;

      // nock을 시위 위치로 직접 이동 (SHAKE 대신 직접 투영)
      arrowState.x[0] = nockTarget.x;
      arrowState.y[0] = nockTarget.y;

      // ── E. 활채 ODE: 복원력 - 시위 반력 ──
      // 시위가 활채를 당기는 힘 = T × cos(시위-q방향 각도) ≈ T × e_sum_x (q ≈ x 방향)
      const F_restore = limbState.F_draw; // 정적 복원력 (q 위치에서)
      // 활채 ODE: 복원력은 활채를 brace로 끌고 가고, 시위 장력은 화살이 연결된 동안 저항
      // F_restore > 0 (만작에서 brace 방향), T가 화살을 가속하면서 활채에 반력
      // 화살이 충분히 가속되면 T가 줄어들어 활채가 빠르게 복원
      // 순가속도 = -(F_restore - T × proj) / m_eff (brace 방향 = q 감소)
      const cosProj = Math.abs(e_sum_x); // 시위력의 q방향 투영
      const F_net_limb = -F_restore; // 복원력만 (T 반력은 nock 구속으로 암묵 처리)
      const k_local = Math.abs(F_restore) / Math.max(Math.abs(q), 0.01);
      const q_accel = F_net_limb / m_eff_limb - 2 * zeta_limb * Math.sqrt(k_local / m_eff_limb) * q_dot;

      // 활채 Verlet
      q += q_dot * dt + 0.5 * q_accel * dt * dt;
      q_dot += q_accel * dt;
      // q를 0 이하로 제한 (brace를 넘어가지 않도록 — 오버슈트는 나중에 처리)
      // 실제로는 오버슈트 허용 (활채가 brace를 지나감)
      if (q < -0.15) q = -0.15; // 물리적 한계

      // ── F. 화살 나머지 노드 적분 (bending + gravity + rest) ──
      // 시위력은 이미 nock 위치 직접 투영으로 적용됨
      // 나머지 노드만 Verlet
      ...
      for (let i = 1; i < N; i++) { // i=0(nock)은 이미 투영됨
        const ax_i = bend.fx[i] / m[i];
        const ay_i = (bend.fy[i] - m[i] * 9.81) / m[i] + (i === restF.nodeIndex && restF.Fy ? restF.Fy / m[i] : 0);
        x[i] += vx[i] * dt + 0.5 * ax_i * dt * dt;
        y[i] += vy[i] * dt + 0.5 * ay_i * dt * dt;
      }
      enforceDistanceConstraints(arrowState, 30);
      for (let i = 0; i < N; i++) {
        vx[i] = (x[i] - x_old[i]) / dt;
        vy[i] = (y[i] - y_old[i]) / dt;
      }
    ...
    const animate = () => {
      ...
      if (isAnimating) {
        ...
        if (anim.phase === "drawing") {
          ...
          window.__DEBUG_HOLDING = (window.__DEBUG_HOLDING || 0) + 1;
          // 만작 상태 저장 (발시 시 사용)
