...
function simulateRelease(params) {
  ...
  for (let step = 0; step <= totalSteps; step++) {
    ...
    if (arrowState.onString) {
      ...
      // ── C. nock 위치: 정적 솔버 보간값 직접 사용 ──
      // computeNockFromStringConstraint(원-원 교점)의 기하학적 비선형 우회
      // q→nockX 매핑이 정적 솔버에서 보장되므로 에너지 전달 정확
      const nockTarget = { x: limbState.nockX, y: limbState.nockY || 0 };
